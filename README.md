...
Feature install scripts run as root, and the base image's `USER` is restored afterwards so the final image keeps the user it declared — a `Dockerfile` that ends in `USER vscode` no longer breaks a feature that needs to write under `/usr/local`.
...
and `-e`). Runtime options such as `--cap-add`, `--device`,
`--device-cgroup-rule`, `--group-add`, `--security-opt`, `--userns`,
